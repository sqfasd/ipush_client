...
- (void)succeed
{
	...
	dispatch_async(delegateQueue, ^{ @autoreleasepool {
		...
		{
			XCLogWarn(@"%@: delegate doesn't implement %@", THIS_FILE, NSStringFromSelector(selector));
		}
...
- (void)failWithError:(NSError *)error
{
	...
	XCLogTrace2(@"%@: %@ %@", THIS_FILE, THIS_METHOD, error);
...
{
	XCLogTrace2(@"%@: %@ %i", THIS_FILE, THIS_METHOD, (int)sdErr);
...
- (XMPPSRVRecord *)processRecord:(const void *)rdata length:(uint16_t)rdlen
{
	...
	if (rdata == NULL)
	{
		XCLogWarn(@"%@: %@ - rdata == NULL", THIS_FILE, THIS_METHOD);
		return nil;
	}
...
- (void)startWithSRVName:(NSString *)aSRVName timeout:(NSTimeInterval)aTimeout
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		XCLogTrace2(@"%@: startWithSRVName:%@ timeout:%f", THIS_FILE, aSRVName, aTimeout);
		...
		dispatch_source_set_event_handler(sdReadSource, ^{ @autoreleasepool {
			...
			XCLogVerbose(@"%@: sdReadSource_eventHandler", THIS_FILE);
		...
		dispatch_source_set_cancel_handler(sdReadSource, ^{ @autoreleasepool {
			...
			XCLogVerbose(@"%@: sdReadSource_cancelHandler", THIS_FILE);
