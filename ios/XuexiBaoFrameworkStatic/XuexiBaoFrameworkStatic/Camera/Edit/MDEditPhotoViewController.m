...
{
    MDCameraCoverView *coverView = [[NSBundle mainBundle] loadNibNamed:@"CameraCover" owner:self options:nil].firstObject;
...
- (void)initRemindLabel
{
    ...
    remindLabel.text = NSLocalizedString(@"editphoto_remind_text", @"");
...
//        if (image.size.width * image.size.height > IMAGE_MAX_PIXEL) {
//            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"alert_title", @"") message:NSLocalizedString(@"alert_image_pixel_toobig", @"") delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", @"") otherButtonTitles:nil];
//            [alert show];
//        }
//        else {
            if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectPhoto:)]) {
                UIImage *finalImage = [self buildImage:image];
                [self.delegate didSelectPhoto:finalImage];
                NSLog(@"confirmPickImage:%@", NSStringFromCGSize(image.size));
            }
//        }
    }];
}
