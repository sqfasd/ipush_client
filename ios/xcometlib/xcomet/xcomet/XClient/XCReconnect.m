...
@interface XCReconnect (PrivateAPI)
...
- (void)xmppStream:(XClient *)sender didReceiveError:(id *)element
{
    // This method is executed on our moduleQueue.
...
static void XMPPReconnectReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info)
{
    @autoreleasepool {
        ...
        XCReconnect *instance = (__bridge XCReconnect *)info;
...
- (void)setupReconnectTimer
{
    ...
    if (reconnectTimer == NULL)
    {
        ...
        dispatch_source_set_cancel_handler(reconnectTimer, ^{
            XCLogVerbose(@"dispatch_release(reconnectTimer)");
...
- (void)setupNetworkMonitoring
{
    ...
    if (reachability == NULL)
    {
        ...
        if (reachability)
        {
            ...
            {
                XCLogWarn(@"%@: %@ - No xmpp moduleQueue!", THIS_FILE, THIS_METHOD);
            }
        }
    }
}
            ...
            dispatch_async(concurrentQueue, ^{ @autoreleasepool {
                ...
                dispatch_async(moduleQueue, ^{ @autoreleasepool {
                    ...
                    if (shouldAttemptReconnect)
                    {
                        ...
                        {
                            //[xmppStream oldSchoolSecureConnectWithTimeout:XClientTimeoutNone error:nil];
                        }
                        else
                        {
                            [xmppStream connectWithTimeout:XClientTimeoutNone error:nil];
                        }
                    }
